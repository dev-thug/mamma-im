...
- 이벤트 데이터 **14개월**(무료 속성 최대값), 사용자 데이터 14개월, 새 활동 시 재설정 켜짐
  (2026-09-09 기본 2개월에서 변경). 무료 속성은 2개월·14개월만 고를 수 있고, 50개월·무기한 보관은
  유료 애널리틱스 360에만 있습니다.
- 영구 보관이 필요하면 BigQuery Export(관리 → 제품 링크 → BigQuery 링크)가 사실상 유일한 방법입니다.
  GA4가 원시 이벤트를 매일 내보내고 BigQuery에 들어간 데이터는 GA4 보관 기간과 무관하게 남지만,
  Google Cloud 프로젝트와 결제 계정이 필요하고 저장 용량만큼 비용이 듭니다.
...
### "육아 에이전트" 검색 수요 — 2026-09-09 확인 결과

Search Console 연결 직후 GA4 쿼리 보고서에서 '에이전트'가 들어간 검색어를 걸러 보니 **지난 28일간
노출 0 / 클릭 0**이었습니다. 같은 기간 전체 노출은 442회였고 대부분 브랜드 검색어("맘마" 389회)였습니다.

이 수치는 mamma.im이 에이전트 관련 검색어에 노출된 적이 없다는 뜻이지, 그 단어의 전체 검색량이 0이라는
증거는 아닙니다. 다만 당시 사이트에 '에이전트'라는 단어가 거의 없었고, 개인 포트폴리오 페이지가 해당
검색어 1위를 유지하고 있던 점을 함께 보면 수요 자체가 작을 가능성이 높습니다.

2026-09-11에 홈 타이틀·AgentSection·FAQ·`/blog/ai-parenting-agent`로 이 단어를 사이트 전반에 실었습니다.
